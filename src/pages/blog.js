  ...
  <Layout>
    <div className='flex justify-around flex-wrap bg-gray-700'>
      {data.allMarkdownRemark.edges.map(post => (
        <div key={post.node.id} className='w-1/3 flex flex-col justify-center items-center p-3 text-gray-200 m-10 mt-0 leading-8 shadow-md'>
          <strong className='text-2xl text-blue-400 mb-8 mt-2'>{post.node.frontmatter.title}</strong>
          <img className='w-3/4 rounded-lg' src={post.node.frontmatter.img} alt='food-img' />
